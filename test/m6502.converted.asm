...
BLTLP:
	LDA (HIGHTR		;FETCH BYTE TO MOVE),Y
	STA (HIGHDS		;MOVE IT IN, MOVE IT OUT.),Y
MOREN1:
 DEY
	BNE	BLTLP
	LDA (HIGHTR		;MOVE LAST OF THE BLOCK.),Y
	STA (HIGHDS),Y
DECBLT:
 DEC	HIGHTR+1
...
MAIN1:
	...
	LDA	LOWTR
	DEY
	SBC (LOWTR		;COMPUTE NEGATIVE LENGTH.),Y
	CLC
...
CHEAD:
	LDY #1
	LDA (INDEX		;ARRIVED AT DOUBLE ZEROES?),Y
	BEQ	LNKRTS
	LDY #4
CZLOOP:
...
FNDLO1:
 LDA	LINNUM
	DEY
	CMP (LOWTR		;COMPARE LOW ORDERS.),Y
	BCC	FLNRTS		;NO SUCH NUMBER.
	BEQ	FLNRTS		;GO TIT.
AFFRTS:
 DEY
	LDA (LOWTR		;FETCH LINK.),Y
	TAX
	DEY
	LDA (LOWTR),Y
...
PLOOP1:
 INY
	...
	LDA (LOWTR		;GET NEXT CHAR. IS IT ZERO?),Y
...
TIMEST:
 LDA	FACMOH,X
	...
	RTS
TIMNUM:
 LDA (INDEX		;INDEX SET UP BY FREFAC.),Y
	JSR	QNUM
	BCC	GOTNUM
FCERR2:
...
STRPR2:
 DEX
	BEQ	PRTRTS		;ALL DONE.
	LDA (INDEX		;PNTR TO ACT STRNG SET BY FREFAC.),Y
	JSR	OUTDO
	INY
	CMP #13
	BNE	STRPR2
...
READ:
	...
	XWD	0o1000,0o251	;LDA #TYA TO MAKE IT NONZERO.
.if (BUFPAG) == 0
INPCON:
 .endif
	TYA
...
DATLOP:
 JSR	DATAN		;SKIP SOME TEXT.
	...
	BEQ	ERRGO5
	INY
	LDA (TXTPTR		;GET HIGH BYTE OF LINE NUMBER.),Y
	STA	DATLIN
	INY
	LDA (TXTPTR		;GET LOW BYTE.),Y
	INY
	STA	DATLIN+1
NOWLIN:
 LDA (TXTPTR		;HOW IS IT?),Y
	TAX
...
VARY0:
	LDY #0
	LDA (INPPTR		;LAST DATA CHR COULD HAVE BEEN),Y
...
GETCMP:
 LDA (ARGMO		;GET NEXT CHAR TO COMPARE.),Y
	CMP (DSCTMP+1	;SAME?),Y
...
LOPFN:
	LDA	VARNAM
	CMP (LOWTR		;COMPARE HIGH ORDERS.),Y
	BNE	NOTIT		;NO COMPARISON.
	LDA	VARNAM+1
	INY
	CMP (LOWTR		;AND THE LOW PART?),Y
...
NOTEVE:
 STWD	HIGHDS		;PLACE TO STUFF IT.
	...
	INY
	LDA	VARNAM+1
	STA (LOWTR		;STORE NAME OF VARIABLE.),Y
	LDA #0
	INY
	STA (LOWTR),Y
	INY
	...
	STA (LOWTR),Y
	INY
	STA (LOWTR		;FOURTH ZERO FOR DEF FUNC.),Y
.if (ADDPRC) != 0
	INY
	STA (LOWTR
	.endif
...
LOPFDV:
 LDY #0
	...
	LDA	VARNAM+1
	CMP (LOWTR		;LOW ORDERS?),Y
	BEQ	GOTARY		;WELL, HERE IT IS !!
NMARY1:
 INY
	LDA (LOWTR		;GET LENGTH.),Y
	CLC
	ADC	LOWTR
	TAX
	INY
...
STOMLT:
 STX	CURTOL
	...
	INY
	.endrepeat
	STA (LOWTR		;SAVE NUMBER OF DIMENSIONS.),Y
LOPPTA:
 LDX #11		;DEFAULT SIZE.
	LDA #0
	BIT	DIMFLG
...
NOTDIM:
 INY
	STA (LOWTR		;STORE HIGH PART OF INDICE.),Y
	INY
	TXA
	STA (LOWTR		;STORE LOW ORDER OF INDICE.),Y
...
INLPNM:
 STA	CURTOL+1
	...
	STA	INDICE+1
	CMP (LOWTR		;COMPARE WITH MAX INDICE.),Y
	BCC	INLPN2
...
UMULT:
	...
	DEY
	LDA (LOWTR		;PUT [LOWTR,Y,Y+1] IN FASTER MEMORY.),Y
UMULTD:
 STA	ADDEND+1
	LDA #16
	STA	DECCNT
...
FNDOER:
 JSR	GETFNM		;GET THE FUNCTION'S NAME.
	...
	PULWD	DEFPNT
	LDY #2
	LDA (DEFPNT		;GET POINTER TO VARIABLE.),Y
...
DEFFIN:
 LDY #0
	...
	STA (DEFPNT		;AND PUT IT BACK IN VARIABLE.),Y
	PLA
	INY
	STA (DEFPNT),Y
	PLA
...
.if (INTPRC) != 0
	...
	INY
DVAR:
	LDA (INDEX1		;IS LENGTH=0?),Y
	BEQ	DVARTS		;YES, RETURN.
	INY
	LDA (INDEX1		;GET LOW(ADR)*),Y
	TAX
	INY
	LDA (INDEX1),Y
...
GRBPAS:
 LDA	GRBPNT+1	;VARIABLE POINTER.
	...
	LDA	HIGHDS+1
	INY
	STA (GRBPNT		;CHANGE ADDR OF STRING IN VAR.),Y
...
CAT:
	...
	STA	STRNG1+1
	LDY #0
	LDA (STRNG1		;GET LENGTH OF OLD STRING.),Y
	CLC
	ADCDY	FACMO
...
MID2:
	...
	CLC
	LDX #0
	SBC (DSCPNT		;GET LENGTH OF WHAT'S LEFT.),Y
...
VAL2:
	STX	INDEX2+1
	LDY #0
	LDA (INDEX2		;PRESERVE CHARACTER.),Y
	PHA
...
.if (ROMSW) != 0
	...
	LDY #0
	TYA
	STA (TXTTAB		;SET UP TEXT TABLE.),Y
	INC	TXTTAB
.if REALIO != 3
	BNE	QROOM
	INC	TXTTAB+1
